...
def create_indexing():
    ...
    if not table_wiki_exists():
        ...
        inv_index_list = indexdict_to_string(index_articles(path_articles, path_titles))

        cursor = db.cursor()
        cursor.executemany(
            '''INSERT INTO wiki(term, counter, indexes) VALUES(?, ?, ?)''', inv_index_list)
        db.commit()
    else:
        return
...
def get_from_wiki(value):

    """ Get all values from the wiki table """
    cursor = db.cursor()
    cursor.execute("SELECT term, counter, indexes FROM wiki WHERE term='{}'".format(value))
    return cursor.fetchall()
